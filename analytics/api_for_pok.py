...
# Определяем модели данных для входных данных
...
class SprintItem(BaseModel):
    ...
    entity_ids: Optional[List[Any]]  # Должен быть список entity_ids
...
def process_data(request: ProcessDataRequest):
    ...
    # Преобразование entity_ids из строки в список (если необходимо)
    ...
    final_results['date'] = final_results['date'].dt.strftime('%Y-%m-%d')  # Преобразуем дату в строку для сериализации
