...
for idx, sprint in sprints.iterrows():
    ...
    # Инициализируем словарь для отслеживания последнего статуса спринта для каждой задачи
    last_sprint_status = {}
    ...
    for current_date in date_range:
        ...
        for _, row in task_changes_on_date.iterrows():
            ...
            if change == sprint_name:
                last_sprint_status[task_id] = 'in_sprint'
                added_tasks.append(task_id)
            else:
                last_sprint_status[task_id] = 'not_in_sprint'
                removed_tasks.append(task_id)

        # Исключаем 'Снятые' задачи
        ...
        # Обработка задач, которые были добавлены и исключены несколько раз в один день
